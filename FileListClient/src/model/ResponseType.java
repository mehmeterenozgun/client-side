...
public class ResponseType {
	...
	public ResponseType(byte[] rawData) {
		if (rawData.length < HEADER_SIZE) {
			throw new InvalidParameterException("Invalid Header: Insufficient data size.");
		}
		...
		if (dataLength > 0) {
			data = Arrays.copyOfRange(rawData, HEADER_SIZE, rawData.length);
		} else {
			data = null; // No data in the response
		}
	}
